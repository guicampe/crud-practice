<script setup>
import EditSubjectModal from '@/components/ui/EditSubjectModal.vue';
...
const emit = defineEmits(["row-click", "delete-subject", "update-subject"]);
...
const handleDelete = async () => {
    await fetchDeleteSubject(deletingItemId.value);
    emit("delete-subject", deletingItemId.value);
    deletingItemId.value = null;
}

const handleUpdated = (updated) => {
     localItems.value = localItems.value.map(i => i.id === updated.id ? updated : i);
                ...
                <div class="relative flex items-center">
                    ...
                    <div v-if="openMenuId === item.id"
                        ...
                        <button class="w-full text-left px-3 py-2 hover:bg-gray-100"
                            @click.stop="editingItem = item; openMenuId = null">
                            Editar
                        </button>
                        <button class="w-full text-left px-3 py-2 text-red-500 hover:bg-red-100"
                            @click.stop="deletingItemId = item.id; openMenuId = null">
                            Excluir
                        </button>
                    </div>
                </div>
